...
export const profileFormSchema = z
  .object({
    username: z
      ...
      .min(3)
      .max(30)
      .regex(/^[a-z0-9._]+$/, "Solo letras en minúscula, números, puntos y guiones bajos."),
    display_name: z.string().max(80),
    bio: z.string().max(400),
    avatar_url: z.string(),
    cover_url: z.string(),

    preferred_sign: z.string(),
    favorite_signs: z.array(z.string()).max(3, "Máximo 3 signos favoritos"),

    show_sun_sign: z.boolean(),
    show_moon_sign: z.boolean(),
    show_favorite_signs: z.boolean(),
    ...
    birth_city: z.string(),
    birth_region: z.string(),
    birth_country: z.string(),
    birth_country_code: z.string(),
    birth_timezone: z.string().min(1, "La zona horaria es obligatoria"),
    birth_latitude: z
      .number({ required_error: "Latitud obligatoria", invalid_type_error: "Latitud inválida" })
      .min(-90)
      .max(90),
    birth_longitude: z
      .number({ required_error: "Longitud obligatoria", invalid_type_error: "Longitud inválida" })
      .min(-180)
      .max(180),
