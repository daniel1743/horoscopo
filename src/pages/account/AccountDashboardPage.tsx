  ...
  return (
    <AccountShell title={`Hola, ${greeting}`} description="Tu espacio personal en Proyecto Astral.">
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Favoritos" value={favorites.data?.length ?? 0} to={routes.favorites} icon="favorite" />
        <StatCard label="Lecturas guardadas" value={readings.data?.length ?? 0} to={routes.savedReadings} icon="tarot" />
        <StatCard label="Signo preferido" value={profile.data?.preferred_sign ?? "—"} to={routes.profile} icon="sun" />
      </div>

      <h2 className="mt-10 font-display text-xl font-semibold text-ink">Todas las secciones</h2>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {accountNav.filter((n) => n.routeKey !== "account").map((item) => (
          <Link
            key={item.routeKey}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-medium text-ink">
                  {item.label}
                  {item.disabled && <span className="ml-2 text-xs text-ink-muted">(no disponible)</span>}
                </div>
...
function StatCard({ label, value, to, icon }: { label: string; value: string | number; to: string; icon: Parameters<typeof Icon>[0]["name"] }) {
  return (
    <Link to={to}>
